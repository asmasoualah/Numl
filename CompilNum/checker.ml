...
| _ -> false ;;
(*************************************************************************)
let rec isCmp e = match e with (* makes sense for float op only *)
Id(op) -> (try
            let s = String.sub op 0 3 in
              ((String.compare s "<F_")=0)|| 
              ((String.compare s ">F_")=0)|| 
              ((String.compare s "<=F")=0)|| 
              ((String.compare s ">=F")=0)||
              ((String.compare s "=F_")=0)||
              ((String.compare s "!=F")=0)||
              ((String.compare s ">B_")=0)||
              ((String.compare s "<B_")=0)|| 
              ((String.compare s ">=_")=0)||
              ((String.compare s "<=_")=0)||
              ((String.compare s "=B_")=0)||
              ((String.compare s "!=_")=0) 
...
| TypeVar(r,_) -> isCmpVar !r
| App(e1,e2) -> isCmp e1
| _ -> false 


and isCmpVar r = match r with
  Some(t) -> isCmp t
| _ -> false ;;
(******************************************************************************)
let rec isFloatOp e = match e with (* makes sense for float op only *)
Id(op) -> (try
            let s = String.sub op 0 3 in
              ((String.compare s "+__")=0)||
              ((String.compare s "-__")=0)|| 
              ((String.compare s "*__")=0)||
              ((String.compare s "/__")=0) 
...
| TypeVar(r,_) -> isFloatOpVar !r
| App(e1,e2) -> isFloatOp e1
| _ -> false 


and isFloatOpVar r = match r with
  Some(t) -> isFloatOp t
| _ -> false ;;
(*****************************************************************************)
...
(***************************************************************************************)
let rec getCmpName e = match e with (* makes sense for float op only *)
  Id(op) -> op
| TypeVar(r,_) -> getCmpVarName !r
| App(e1,e2) -> getCmpName e1
| _ -> raise (Error "???getCmpName")


and getCmpVarName r = match r with
  Some(t) -> getCmpName t
| _ -> raise (Error "???getCmpVarName") ;;

(******************************************************************************************)
let rec getFloatOpName e = match e with (* makes sense for float op only *)
  Id(op) -> op
| TypeVar(r,_) -> getFloatOpVarName !r
| App(e1,e2) -> getFloatOpName e1
| _ -> raise (Error "???getFloatOpName")


and getFloatOpVarName r = match r with
  Some(t) -> getFloatOpName t
| _ -> raise (Error "???getFloatOpVarName") ;;
(*****************************************************************************************)
...
let updateValEnv id vEnv t = 
  ...
  let v = if ((String.compare x "+__")=0) then newPlusVar s u p else
          ...
          if ((String.compare x "sqrt__")=0) then newSqrtVar s u p else 
...
| Sqrt(e1,_,_,_) -> (doesNotOccurs v e1)
...
let rec typeApp e1 e2 env valEnv =
  ...
  let argType = getArgType t1' env in
    (try (ltType t2' argType ;
	  ...
	  if (isCmp e1) then 
		(match t2' with
			TFloat(_) -> cmpTypEnv := setEnv (getCmpName e1) "FLOAT" !cmpTypEnv 
		      | _ -> cmpTypEnv := setEnv (getCmpName e1) "INT" !cmpTypEnv
	        ) ; 
